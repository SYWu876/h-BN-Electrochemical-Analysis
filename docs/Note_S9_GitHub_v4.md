...
- The **GCD** branch is represented by raw traces, preprocessing diagnostics, flattest-window masks, bootstrap outputs, residual tables, and final summary tables regenerated by one shared algorithm for both the archived h-BN dataset and related rerun datasets. The Csp step excludes the early IR-drop region, scans candidate post-IR discharge segments, and selects the flattest quasi-linear segment by local stability cost, slope variation, absolute discharge slope, and linear-regression error on the smoothed voltage. The reported `Csp = J / |dV/dt|` is then fitted from the raw voltage points inside that data-selected window; `Rs` is reported afterward as an effective masked-domain descriptor using the manuscript current-density convention, not as an independently free nonlinear-fit parameter.
